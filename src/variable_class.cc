...
void Variable::deletevar(void)
		{
			...
			if(curr->prev) // YES-@-??
			{
				...
				if(temp->next)  //   ...1->@->2->...    1->2->   free(@)
				{
					...
					if(temp->cp)
						while(--temp->datanum) delete[] temp->cp[(long)temp->datanum];					delete[] temp->cp;
				...
				{	
					...
					if(curr->next->cp)
						while(--curr->next->datanum>=0) delete[] curr->next->cp[(long)curr->next->datanum];					delete[] curr->next->cp;
			...
			{                   //       |                |
				...
				{	
					...
					if(temp->cp)
						while(--temp->datanum>=0) delete[] temp->cp[(long)temp->datanum];					delete[] temp->cp;
...
void Variable::deletename(char *n)
		{
			...
			while(curr->next)
			{				
				if(cmp(curr->name,n)){ deletevar(); break; }
				curr = curr->next;	
			}
		}
...
void Variable::deletenamescope(char *n,char *sp)
		{	
			...
			while(curr->next)
			{				
				if(cmp(n,curr->name) && cmp(sp,curr->Scope)){ deletevar(); break; }
			 	curr = curr->next;
			}
		}
...
int Variable::deletescope(char *sp)
		{	
			...
			while(curr->next)
			{
				...
				if(cmp(curr->Scope,sp)){ deletevar(); curr = root; cnt++; }
				if(curr->next)
					curr = curr->next;
			}
