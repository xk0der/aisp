...
int process(char *t, char *arg, int &i,int &a)
{
	...
	int j=0;
	t = trim(t);
	while(strcmp(tstr[j],"@@")!=0)	{	if(strcmp(tstr[j],t)==0){ token = j; break; }   j++;	}
	...
	switch(token)
	{
		...
		case _dim:
				...
				switch(varg[1][0])
				{
					case 'I': tmpl = (long int) _a_integer;
							break;
					case 'F': tmpl = (long int) _a_float;
							break;
					case 'D': tmpl = (long int) _a_double;
							break;
					case 'L': tmpl = (long int) _a_long;
							break;
					case 'S': tmpl = (long int) _a_string;
							break;
					case 'B': tmpl = (long int) _a_bool;
							break;
					case 'N': tmpl = (long int) _a_double;
							break;				}
		...
		case _mul:
				...
				if(!var.varinscope(varg[0],scope)){ error(ERR_VARDEC,"@mul()",i);}
		...
		case _if:
				...
				{					flag = NOTSET;
					while(cline<total_lines)
					{
						...
						if(script_inif == tmpi){flag = SET; break;}
					}
		...
		case _and:
				...
				trim(varg[0]); trim(varg[1]);
				ucase(varg[0]); ucase(varg[1]);
		...
		case _while:
				...
				if(cmp(varg[0],"FALSE"))
				{
					...
					while(cline<total_lines)
					{
						...
						if(script_inloop==tmpi){ flag = SET; break; }
					}
		...
		case _switch:
				...
				if(!label.islabel(temp3))
				{
					...
					while(cline<total_lines)
					{
						...
						if(script_inswitch==tmpi-1){ flag=SET; break;}
					}
					i = strlen(exp)+2;
		...
		case _for:
				...
				if(!var.varinscope(varg[0],scope)) error(ERR_VARDEC,"@for()",i);									var.set(varg[0],varg[1]);
				script_infor++;
				forstack.push(cline);
				tmpf = 1; // Step
		...
		case _until:
				...
				script_inloop--;
				if(cmp(varg[0],"FALSE"))
				{cline = loopstack.pop() - 1; i = strlen(exp)+2;}				if(script_inloop<0) error(ERR_MISSINGDO,"@until()",i);
				break;
		case _till:
				...
				script_inloop--;
				if(cmp(varg[0],"TRUE"))
				{cline = loopstack.pop() - 1; i = strlen(exp)+2; }				if(script_inloop<0) error(ERR_MISSINGDO,"@till()",i);
				break;
		case _term:
		...
		case _pop:
				...
				if(ScriptStack.okay()){i = rexp(temp3,i,a); scriptStackOkay = TRUE;}	 
				else{ i = rexp(" ",i,a); scriptStackOkay	= FALSE;	}	
				break;
		case _stackokay:
	...
	{	delete[] varg[j]; varg[j]=NULL;}
	delete[] varg; varg=NULL;
	narg = 0;
